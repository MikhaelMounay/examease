    ...
    return (
        <li className="group relative mb-3">
            <NavLink
                to={props.linkTo}
                className={({ isActive }) =>
                    "hover:bg-primary hover:text-textColorLight block h-full truncate rounded p-2 py-3 duration-200 " +
                    (isActive ? "bg-secondary text-textColorLight" : "")
                }
            ...
            >
                <div className={"flex items-center " + (props.isSidebarCollapsed ? "justify-center" : "")}>
                    ...
                    {!props.isSidebarCollapsed && <div className="truncate">{props.label}</div>}
                </div>
            </NavLink>
